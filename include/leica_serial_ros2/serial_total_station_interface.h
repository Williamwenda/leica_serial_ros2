/**
 * \file serial_total_station_interface.h
 * \author Wenda Zhao
 * \date 05.11.2025
 * \brief Header file containing the required defintion for the serial total station interface
 */
...
/**
 * @brief Serial interface for the Leica total station
 */
class SerialTSInterface: public TSInterface {
 public:
  ...
  /**
   * @brief Tries to open a serial connection to the total station
   *        Initializes the serial connection to the total station
   *        and the timer to detect if no message are received anymore.
   *
   * @param comport The COM port device path as std::string (e.g., "/dev/ttyUSB0")
   */
  void connect(std::string comport);

 private:
  /**
   * @brief Starts the serial reader.
   */
  void startReader();
